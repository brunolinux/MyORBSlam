  ...
  while((int)clusters.size() < m_k)
  {
    ...
    for(fit = pfeatures.begin(); fit != pfeatures.end(); ++fit, ++dit)
    {
      if(*dit > 0)
      {
        ...
        if(dist < *dit) *dit = dist;	// bruno add: nearest centroid --> smallest distance 
      }
    }
...
void TemplatedVocabulary<TDescriptor,F>::transform(
  ...
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  v.clear();		// bruno add: map from world id ---> weight
  fv.clear();		// bruno add: map from node id  ---> vector of int
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)  // bruno add: iterate each descriptor
    {
      WordId id;
      NodeId nid;
      WordValue w; 
      ...
      transform(*fit, id, w, &nid, levelsup); // bruno add: search in the tree (already constrcuted by ~10000 images)
      
      if(w > 0) // not stopped
      { 
        v.addWeight(id, w);				// word id --> weight
        fv.addFeature(nid, i_feature);  // node id --> vector of i_feature (index of feature)
      }
    }
...
bool TemplatedVocabulary<TDescriptor,F>::loadFromTextFile(const std::string &filename)
{
    ...
    stringstream ss;
    ss << s;
    ss >> m_k;		// node number per parent node
    ss >> m_L;		// level number
    int n1, n2;	
    ss >> n1;		// score type: 0 --> L1, 1 --> L2
    ss >> n2;		// weight type
    ...
    {
        string snode;
        getline(f,snode);						// bruno add: one line --> one node
        stringstream ssnode;
        ssnode << snode;
        ...
        int pid ;
        ssnode >> pid;							// 1st: parent id
        m_nodes[nid].parent = pid;
        m_nodes[pid].children.push_back(nid);

        int nIsLeaf;
        ssnode >> nIsLeaf;						// 2st: whether is leaf node

        stringstream ssd;
        for(int iD=0;iD<F::L;iD++)				// 32 (3th, 4th, ..., 34th) * uchar (8 bit) = 256 bit desc
        {
            string sElement;
            ssnode >> sElement;
            ssd << sElement << " ";
        ...
        ssnode >> m_nodes[nid].weight;			// last: weight value
